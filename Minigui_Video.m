...
if Fcell>length(CumlDistance)
    ed=msgbox('Incorrect input value. The number of cells to display exceeds the total number of cells in the domain','FluOil Error: Incorrect input value','error');
    set(ed, 'WindowStyle', 'modal');
    uiwait(ed);
    return
end     
...
zt=Videodata.zt;
 xx=Videodata.xx;
    ...
    for t=0:round(Videodata.Time_step_frames*3600):length(time)*Dt
        ...
        counter=counter+1;
        if t==0
            Message= msgbox('Your video is now being created. Please wait','FluOil is creating the video','help');
            pause(2.2)
            try
                delete(Message)
            catch
...
axis off
%% Save the movie %ZZ movie2avi has been removed, and changed to be VideoWriter
handleResults=getappdata(0,'handleResults');
pathname=getappdata(handleResults,'pathname');

v = VideoWriter([pathname,'animation.avi']);
v.FrameRate=2; %ZZ 2 frames per second
open(v);
writeVideo(v,M);
close(v);

%ZZ movie2avi(M,[pathname,'animation' '.avi'], 'compression', 'None','fps',2,'quality',100);%Saving the movie
Message= msgbox('Your video is completed. It is available in the results folder','FluOil','help');
pause(3)
try
    delete(Message)
catch
