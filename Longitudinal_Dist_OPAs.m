function varargout = Longitudinal_Dist_OPAs(varargin)
...
gui_State = struct('gui_Name',       mfilename, ...
    ...
    'gui_OpeningFcn', @Longitudinal_Dist_OPAs_OpeningFcn, ...
    'gui_OutputFcn',  @Longitudinal_Dist_OPAs_OutputFcn, ...
    'gui_LayoutFcn',  [] , ...
    'gui_Callback',   []);
...
% --- Executes just before Longitudinal_Dist_OPAs is made visible.
function Longitudinal_Dist_OPAs_OpeningFcn(hObject, ~, handles, varargin)
...
function varargout = Longitudinal_Dist_OPAs_OutputFcn(~, ~, handles)
diary off
...
if isempty(SetTime)||isnan(SetTime)
    ed=msgbox('Empty input field. Please make sure all required fields are filled out correctly ','FluOil Error: Empty fields','error');
    try
...
if SetTime<=0
    ed=msgbox('Incorrect negative or zero value. Please make sure all required fields are filled out correctly','FluOil Error: Incorrect negative or zero value','error');
     try
    rmappdata(handleResults,'SetTime') 
    catch
        %If the handle does not exist, dont do anything
...
if SetTime>str2double(num2str(round(ResultsSim.time(end)*10/3600)/10)) 
  ed = errordlg('Time exceeds simulation time','FluOil Error: incorrect input value');
   try
    rmappdata(handleResults,'SetTime') 
    catch
        %If the handle does not exist, dont do anything
...
if (CalculateEggs_at_risk_hatching==1)&(SetTime<str2double(num2str(ResultsSim.T2_Hatching)))
    ed = errordlg('The time is less than the estimated hatching time, the eggs at risk of hatching cannot be calculated','FluOil Error: incorrect input value');
    try
        rmappdata(handleResults,'SetTime')
    catch
        %If the handle does not exist, dont do anything
