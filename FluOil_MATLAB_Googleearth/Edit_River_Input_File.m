%%%:::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::%
%%                       Edit or import FluOil River Input data           %
%:::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::%
%-------------------------------------------------------------------------%
% This function is used to import river input data into FluOil. Currently %
...
%        temperature.
% Outputs: FluOil River input file(s)
    ...
    if FileName ~= 0
        ...
        try
            ...
            if size(Riverinputfile_hdr) ~= [1 9]
                ed = msgbox('Incorrect river input file, please select another file','FluOil Error','Error');
                set(ed, 'WindowStyle', 'modal');
                uiwait(ed);
                return
...
if PathName==0 %if the user pressed cancelled, then we exit this callback
    ...
    if FileName~=0
        % Load River input file
        m=msgbox('Please wait, loading HEC-RAS project file...','FluOil');
...
set(handles.TempPlot,'Visible','off');
    ...
    function [RC]=loadsHECRAS(strFilename)
        ...
        try
            ...
            % Identify whether we are importing data for FluOil simulation or HEC-RAS
            % performance evaluation
                        importData = get(handles.Import_HECRAS_panel_button,'Value');
            if importData == 1 %Importing data for FluOil simulation
            ...
            ed = errordlg('FluOil cannot load you HEC-RAS project. Please check HEC-RAS files and try again','Error');
            set(ed, 'WindowStyle', 'modal');
            uiwait(ed);
        end
    end %loadsProfiles
...
%===========================================================================================
hFluOilGui = getappdata(0,'hFluOilGui');
HECRAS_data=getappdata(hFluOilGui,'inputdata'); %0 means root-->storage in desktop
...
hFluOilGui = getappdata(0,'hFluOilGui');
handlesmain = getappdata(hFluOilGui, 'handlesmain');
...
%    m = msgbox('Please import data and try again','FluOil error','error');
% uiwait(m)
% return
% else
...
% else
% m = msgbox('Please select a variable to plot','FluOil error','error');
% uiwait(m)
% return
% end
% date=arrayfun(@(x) datenum(x.Date,'ddmmyyyy HHMM'), HECRAS_data.Profiles);
...
%         %msgbox('Feature under development, no available','FluOil message')
...
%         set(handles.Set_up_spawning_time,'Visible','on')
    ...
    function [date_axis, Yylabel] = plotTS(handles, var)
        ...
        if isempty(HECRAS_data)
            % If HEC-RAS data store in FluOil is not found
            m = msgbox('Please import data and try again','FluOil error','error');
            uiwait(m)
            return
        else
    ...
    function plot_obs_data(handles)
        ...
        hFluOilGui  = getappdata(0,'hFluOilGui');
        obs_data    = getappdata(hFluOilGui,'obsdata');
        h = handles.Plot_Hydrograph;
        
        if ~isempty(obs_data)
            % Observed data was loaded in FluOil
...
%% Check user loaded HEC-RAS project
    function error_load()
        ed = errordlg('Please load the HEC-RAS project and import the data into FluOil','Error');
        set(ed, 'WindowStyle', 'modal');
        uiwait(ed);
    end
%==========================================================================
%% Import Data
    function import_data()
        ...
        if lngProfile==0 % If unsteady state-->Multiple profiles
            ...
            %     msgbox('Feature under development, no available','FluOil message')
            %     return
        else %ZZ Read HEC-RAS profiles until a selected profile
       %ZZ     HECRAS_data.Profiles(1).Riverinputfile = ...
       %ZZ         Extract_RAS(handles.strFilename,handles,1); sta
	   %ZZ-Start added by referring to unsteady HEC-RAS
            Profiles=get(handles.popup_HECRAS_profile,'string');
            %% Check user loaded HEC-RAS project
            if  (strcmp(Profiles,' ') == 1)
                ed = errordlg('Please load the HEC-RAS project and import the data into FluOil','Error');
            ...
            %%===========
            Profiles=Profiles(3:get(handles.popup_HECRAS_profile,'Value'));%Read until the popup profile to the end
            % HECRAS_data.Profiles.Date=Profiles;
            HECRAS_data.Profiles(length(Profiles),1).Riverinputfile=NaN;
            waitbar(0,h,['Please wait....' 'Importing HEC-RAS data']);
            for i=1:length(Profiles)
...
%% Import Observed Time Series Data
    function import_OBS()
        ...
        if ~strcmp(fileName,'')
            ...
            hFluOilGui = getappdata(0,'hFluOilGui');
            setappdata(hFluOilGui,'obsdata',OBS_data);
        end
    end
...
switch str{val};
    ...
    case 'Constant temperature in time'
        msgbox('This feature is currently under development, stay tuned to new updates','FluOil','Feature under development');
...
elseif FileName ~= 0
    % Load River input file
    m = msgbox('Please wait, loading Observed Data...','FluOil');
