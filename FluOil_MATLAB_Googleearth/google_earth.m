...
% Last Modified by GUIDE v2.5 19-Jul-2019 16:41:16
...
if T2_Gas_bladder>0
    set(handles.FluOil_results_menu,'String',Menu_labels);
end
%%=========================================================================
...
if isfield(handles, 'inputfile')==0%This is for results files from previous FluOil versions
...
set(handles.Export_FluOil_results_panel,'Visible','on');
%%
handles.x=x;
handles.y=y;
...
%%
ed = msgbox('A kml file has been created, please check the results folder','FluOil message');
set(ed, 'WindowStyle', 'modal');
uiwait(ed);
end %function
...
%T2_Hatching=ResultsSim.T2_Hatching;
if isfield(ResultsSim, 'T2_Hatching')==0%This is for results files from previous FluOil versions
    Temp=ResultsSim.Temp;
...
if length(Xsusp)>5000
    ...
    %%
    ed = msgbox(['The number of eggs in suspension is too large, the egg location has been downsample by ' num2str(round(length(Xsusp)/5000)) ' and the minimum and maximum egg location has been preserved'],'FluOil message');
    set(ed, 'WindowStyle', 'modal');
    uiwait(ed);
end
...
if length(Xbot)>5000
    ...
    %%
    ed = msgbox(['The number of deposited OPAs is too large, the OPA location has been downsample by ' num2str(round(length(Xbot)/5000)) ' and the minimum and maximum OPA location has been preserved'],'FluOil message');
    set(ed, 'WindowStyle', 'modal');
    uiwait(ed);
end
