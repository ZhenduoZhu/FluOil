%%%:::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::%
%%                 OPA vertical concentration distribution                %
%:::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::%
%-------------------------------------------------------------------------%
% This function is part of the post-processing tools included in FluOil.  %
% This tool is used to generate a vertical concentration distribution of  %
% OPAs at a given downstream distance                                     %
%-------------------------------------------------------------------------%
%:::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::%
function varargout = OPA_vertical_concentration(varargin)
% OPA_VERTICAL_CONCENTRATION MATLAB code for OPA_vertical_concentration.fig
%
...
gui_State = struct('gui_Name',       mfilename, ...
    ...
    'gui_OpeningFcn', @OPA_vertical_concentration_OpeningFcn, ...
    'gui_OutputFcn',  @OPA_vertical_concentration_OutputFcn, ...
    'gui_LayoutFcn',  [] , ...
    'gui_Callback',   []);
...
%% ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
function OPA_vertical_concentration_OpeningFcn(hObject, ~, handles, varargin)
...
function OPA_Vertical_Concentration_Distribution(hObject, eventdata, handles)
...
if str2double(get(handles.X_editBox,'String'))<=0||str2double(get(handles.Nlayers_editBox,'String'))<=0
    msgbox('Incorrect negative or zero value. Please make sure all required fields are filled out correctly','FluOil Error: Incorrect negative or zero value','error');
    return
end
%==========================================================================
%% From results GUI
...
[~,b] = size(X);
check = 0;
%% Have all the OPAs arrive to distance X???
for i=1:b
...
%% Display error if not all the OPAs arrived to distance selected by user
if check==0
    ed = errordlg('No OPAs have arrived downstream distance X, please check the OPA vertical distribution closer to the spawning location or run the model for a longer time','Error');
    set(ed, 'WindowStyle', 'modal');
    uiwait(ed);
else
    z_over_H = Nodes;
    ...
    figure('Name','OPA vertical concentration distribution','Color',[1 1 1],...
    ...
    xlabel('log C_z/C [%]');
    ylabel('z/h+1');
    legend('FluOil simulation','Location','NorthWest')
    %% Grid ON-OFF
    ...
    %% Message
    h = msgbox([num2str(sum(N)) ' OPAs passed by ' num2str(Dist_X/1000) ' km downstream from the virtual spawning location during the simulation time'],'FluOil Message');
    diary off
end
close(handles.figure1);

function varargout = OPA_vertical_concentration_OutputFcn(~, ~, handles)
diary off
varargout{1} = handles.output;

function X_editBox_Callback(hObject,eventdata, handles)
handles.eddit = 0;
OPA_Vertical_Concentration_Distribution(hObject, eventdata, handles);
