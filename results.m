%%:::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::%
%%                    FluOil Post-processing tools                        %
%%           Generate video or figures to analize FluOil results          %
%:::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::%
%-------------------------------------------------------------------------%
% This is the results GUI, it is used inside FluOil to generate different %
% plots to analyze FluOil results.                                        %
%-------------------------------------------------------------------------%
...
set(handles.Postprocessing_option(2),'string','Vertical position of the centroid of the OPAs mass');
...
switch Tab
    ...
    case 'Temporal'
        for i=length(Postprocessing_option):-1:1
            if  strcmp(Postprocessing_option(i),'OPAs travel-time distribution')
                travel_time();
            end
        end
        %%=================================================================================================
    case 'Mixed'
        for i=length(Postprocessing_option):-1:1
            if  strcmp(Postprocessing_option(i),'Evolution of a mass of OPAs')
                snapshotPlotDt();
            end
            if  strcmp(Postprocessing_option(i),'Summary of temporal and spatial evolution of the OPAs mass')
                temporal_and_spatial_dispersion();
            end
            if  strcmp(Postprocessing_option(i),'3D animation of OPAs transport (video)')
                Minigui_Video();
            end
...
figure('Name','Normalized vertical position of the centroid of the OPAs mass','NumberTitle','off',...
...
text(double(Xi*0.9/1000),1.08, {'OPAs','drift','started','here'},'HorizontalAlignment','center','FontName','Arial')
%%
...
% if round(T2_Hatching*60*60)>time(end) %ZZ check later
    % ed = errordlg('ZZ: check','Error');
    % set(ed, 'WindowStyle', 'modal');
    % uiwait(ed);
% end
diary off
...
set(0,'Units','pixels') ;
scnsize = get(0,'ScreenSize');
figure('Name','Summary of temporal and spatial evolution of the OPAs mass','Color',[1 1 1],...
...
%%
    function PlotingZ()
        ...
        xlabel('Time since spill [h]','FontName','Arial','FontSize',11);
        set(gca,'XLim',[0 max(time)]);
        ylim([-0.1 1.1])
    end%PlotingZ
%%
...
if round(TimeToHatch*60*60)>time(end)
    % Construct a questdlg with two options
choice = questdlg('Would you want to plot vertical distribution of OPAs at the end of simulation period?', ...
...
switch choice
    ...
    case 'at a different time'
        TimeToHatch=str2double(inputdlg('time(h)',...
              'Time', [1 10])); 
          if round(TimeToHatch*60*60)>time(end)
              errordlg('Input time is greater than simulated time','FluOil Error');
          end
...
%This was designed for time to hatch, now we use it for different times
    ...
    set(0,'Units','pixels') ;
    scnsize = get(0,'ScreenSize');
    figure('Name','Percentage of OPAs distributed in the vertical','Color',[1 1 1],...
    ...
    ylabel('Normalized vertical position (z/h+1) [ ]','FontName','Arial','FontSize',12);
    xlabel('Percentage of OPAs [%]','FontName','Arial','FontSize',12);
    set(gca,'TickDir','in','TickLength',[0.021 0.021],'FontName','Arial','FontSize',12)
    %% Text
...
%%
if check==0
    ed = errordlg('No OPAs have arrived to distance X, please check the distribution of OPAs closer to the spill location or run the model for a longer time','Error');
    set(ed, 'WindowStyle', 'modal');
    uiwait(ed);
else
    Z_Dist_X=(Z_Dist_X+h)./h;
    N=histc(Z_Dist_X,0:0.05:1);N=N(1:end-1)';%N(end) is number of OPAs located above water depth--> none
    %% Plot
    set(0,'Units','pixels') ;
    scnsize = get(0,'ScreenSize');
    figure('Name','Percentage of OPAs distributed in the vertical','Color',[1 1 1],...
...
%%
if check==0
    ed = errordlg('No OPAs have arrived downstream distance X, please check the distribution of OPAs closer to the spill location or run the model for a longer time','Error');
    set(ed, 'WindowStyle', 'modal');
    uiwait(ed);
else
    Z_Dist_X=(Z_Dist_X+h)./h;
    ...
    hold on
    ylabel('Percentage of OPAs [%]','FontName','Arial','FontSize',12);
    xlabel('Time since spill [h]','FontName','Arial','FontSize',12);
    set(gca,'TickDir','in','TickLength',[0.021 0.021],'FontName','Arial','FontSize',12)
    ax1=gca;
    %%
    %% Time to hatch
    legend('Deposition','In suspension','location','NorthWest');
    TimeToHatch = HatchingTime(Temp,specie);
    %
    ylimits=get(ax1,'YLim');
    N=Nsusp+Nbot;
    ...
    %% Message
    h = msgbox(['At the end of the simulation time ' num2str(sum(N)) ' OPAs passed by ' num2str(dX/1000) ' km'],'FluOil Message');
end
end% travel time
...
%% Sub-function-->Distribution of eggs at hatching time
    function Distribution_eggs_hatch(SetTime)
        ...
        %%ZZ Define OPAs in suspension and deposition
		alive=ResultsSim.alive;
        Xsusp=X_at_Time(alive(TimeIndex,:)==1);
		Xbot=X_at_Time(alive(TimeIndex,:)==0);
        Xsuspmean=nanmean(Xsusp);
        Xbotmean=nanmean(Xbot);
		% Nsusp=X_at_Time(Z_at_Time_H>d/2);
        % Nbot=X_at_Time(Z_at_Time_H<=d/2);
        %%
        ...
        bids=(edges(1:end-1)+edges(2:end))/2;bids=bids';
        %%
        Nbot=histc(Xbot,edges,1);   %ZZ Added dim=1 (operates along column) to aviod a bug when there is only one deposited particle 
        Nbot=Nbot(1:end-1);%here we dont include numbers greater than the max edge
        try
            Nsusp=histc(Xsusp,edges,1); %ZZ Added dim=1 (operates along column) to aviod a bug when there is only one particle in suspension
            Nsusp=Nsusp(1:end-1);%here we dont include numbers greater than the max edge
            if isempty(Nsusp)
                Nsusp=Nbot*0;
            end
        catch
        ...
        if CalculateEggs_at_risk_hatching==1
            ...
            ylabel(axes2,'Cumulative pct. of OPAs in suspension [%]','FontName','Arial','FontSize',12);
            
            %%
            idtext=find(cdf_Nsusp>=max(cdf_Nsusp),1,'first');
            text(double(bids(idtext)*0.88),94, {['Approximately ' num2str(round((cdf_Nsusp(end)*10)/10)),'% of OPAs are'],' at risk of '},'HorizontalAlignment','center','FontName','Arial')
...
%% Change units
    function [Convert_km_to_miles,xlabel_text,StringStats]=setupUnits(X_at_Time,Xsuspmean,Xbotmean)
        handleResults=getappdata(0,'handleResults');
        ...
        if Menu.english_Units
            ...
            StringStats={'Stats' ['Mean=',num2str(round(10*Convert_km_to_miles*(nanmean(X_at_Time)))/10),' miles'],['In Suspension Mean=',num2str(round(10*Convert_km_to_miles*Xsuspmean)/10),' miles'],['Deposition Mean=',num2str(round(10*Convert_km_to_miles*Xbotmean)/10),' miles'],['LE=',num2str(round(10*Convert_km_to_miles*(max(X_at_Time)))/10),' miles'],['TE=',num2str(round(10*Convert_km_to_miles*(min(X_at_Time)))/10),' miles']};
        else
            ...
            %StringStats={'Stats' ['Mean=',num2str(round(10*nanmean(X_at_Time)-X(1,1)/1000)/10),' km'],['LE=',num2str(round(10*max(X_at_Time)-X(1,1)/1000)/10),' km'],['TE=',num2str(round(10*min(X_at_Time)-X(1,1)/1000)/10),' km']};
             StringStats={'Stats' ['Mean=',num2str(round(10*nanmean(X_at_Time))/10),' km'],['In Suspension Mean=',num2str(round(10*Xsuspmean)/10),' km'],['Deposition Mean=',num2str(round(10*Xbotmean)/10),' km'],['LE=',num2str(round(10*max(X_at_Time))/10),' km'],['TE=',num2str(round(10*min(X_at_Time))/10),' km']};
        end
    end
...
switch Tab
    case 'Vertical'
        ...
        set(handles.Postprocessing_option(1),'string','Distribution of OPAs over the water column' )
        set(handles.Postprocessing_option(2),'string','OPAs vertical concentration distribution' )
    ...
    case 'Temporal'
        ...
        set(handles.Postprocessing_option(1),'string','OPAs travel-time distribution' )
    ...
    case 'Mixed'
        ...
        set(handles.Postprocessing_option(1),'string','Evolution of a mass of OPAs' )
        set(handles.Postprocessing_option(2),'string','Summary of temporal and spatial evolution of the OPAs mass');
        set(handles.Postprocessing_option(3),'string','3D animation of OPAs transport (video)');
        set(handles.Postprocessing_option(4),'string','Calculate the percentage of OPAs at a given location, depth, and time');
