%% FluOil main function: FluOilgui.m
%%:::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::%
%%                       MAIN FUNCTION                                    %
%                                                                         %
...
switch Inv_mod_status
    ...
    case 'on' % If inverse modeling is activated
        ...
        choice = questdlg('You are about to start an inverse simulation of drifting OPAs, are you sure you want to continue?'...
            ,'Warning','Yes','No','Yes');
        switch choice
            case 'Yes'
                %Continue
...
%% Iniciate Waitbar
h = waitbar(0,'Initializing variables...','Name','OPAs drifting...',...
    'CreateCancelBtn',...
...
%%ZZ Switch to turn on or off deposition/resuspension model
alivemodel = 0;  %ZZ if alivemodel=1 means OPAs would not deposit; alivemodel=0 means OPAs would deposit when they hit bottom and can be resuspended if bed shear stress exceeded critical shear stress
Exit = 0; %If we exit the code
OPApassDS = 0; %ZZ Initially no OPA has passed downstream boundary
% =======================================================================
...
%%ZZ	HECRAS_time_index=HECRAS_data.HECRASspawiningTimeIndex;%HEC-RAS spawning time index, different from
   ...
   %Time difference between HEC-RAS and FluOil
   HECRAS_FluOil_Timediff=(SpawningTime-HECRAS_StartingTime)*24*60*60;%in seconds
...
% switch Larvaemode %:Updated TG May,2015
    ...
    % case 'on'
        ...
        % if handles.userdata.Totaltime>(handles.userdata.Max_Sim_Time+0.000001)
            % choice = questdlg('Error, the simulation time overpasses the estimated time to reach gas bladder stage, do you want FluOil to use the estimated time to reach gas bladder stage as the simulation time?'...
    ...
    % case 'off'
        ...
        % if handles.userdata.Totaltime>(round(handles.userdata.Max_Sim_Time*100)/100)+0.000001
            % choice = questdlg('Error, the simulation time overpasses the estimated hatching time, do you want FluOil to use the hatching time as the simulation time?'...
                % ,'Simulation time Error','Yes','No','Yes');
            % switch choice
                % case 'Yes'
                    % Totaltime = handles.userdata.Max_Sim_Time;
...
%% In case of deposition/resuspension model active ======================================
if alivemodel==0
    %Check int 8 for this case
    Dead_t = cell;
    touch = zeros(Steps,Eggs,'int8');%counter to calculate how many OPAs touched the bottom every time step
...
%ZZ waitbar(0,h,['Please wait....' 'Running growth model']);
...
% Get data from GUI
switch str{val};
    case 'Use constant OPA settling velocity and critical shear stress' %%model would use a constant settling velocity
        D = str2double(get(handles.ConstD,'String')); %Diameter in mm
        D = single(D*ones(length(time),1));
        Vs = str2double(get(handles.ConstVs,'String')); %ZZ settling velocity in mm/s
		Tauc = str2double(get(handles.ConstTauc,'String')); %ZZ critical shear stress in Pa
		Ustarc = sqrt(Tauc/1000.0); %ZZ critical shear velocity
        %ZZ Rhoe_ref = single(str2double(get(handles.ConstRhoe,'String')));
        %ZZ Rhoe_ref = Rhoe_ref*ones(length(time),1,'single');
        Rhoe_ref = 1000.0*ones(length(time),1,'single'); %ZZ density=1000, not used
        Tref = str2double(get(handles.Tref,'String'));
    case 'OPA kinematics (not activated)' %ZZ not used
        Tref = 22; %C
        [D,Rhoe_ref] = EggBio;
        %% Are we doing inverse modeling? TG
...
%ZZ fall velocity of OPAs is input data
Vzpart = single(-Vs/1000);%ZZ Vs is given in mm/s, then we convert it to m/s
%======================================================================
...
if Dt>minDt
    waitfor(msgbox(['The selected time step is too large, A value of Dt=', num2str(minDt), ' seconds it is going to be used in the simulation'],'FluOil Warning','warn'));
    delete(h)
...
%% Lagrangian movement of OPAs (Please reffer to Jump function below)
Warning_flag=0;
Warning_flag2=0; %ZZ
Jump;
%========================================================================
...
% 3.  Jump                                                                %
    function Jump
        ...
        for t=2:Steps                    
            ...
            %%
            %a=Z(t-1,:)>0;
            if alivemodel==0 %If we are simulating OPAs deposition/resuspension
                a = alive(t-1,:) == 1; %a = 1 for OPAs that are in waterbody in the previous time step
            else
                a = Z(t-1,:)' > -2*H;
            end
            ...
            % Reflecting Boundary: If OPAs are located outside the
            % upstream boundary condition
            check = X(t,a);
            if Inv_mod==1
              check(check<d/2) = d-check(check<d/2);
            if (max(check<d/2)>0) & Warning_flag==0   %ZZ: if any OPA passed upstream boundary
                hh=msgbox('Some OPAs crossed the upstream boundary and where bounced back to the domain','FluOil Warning','warn');
                pause(2)
            ...
            elseif sum(check<d/2)>=1 && Warning_flag2==0   %ZZ: if any OPA passed downstream boundary
				hhh=msgbox('OPAs are outside the domain','Please review river input file or decrease the simulation time.','warn');
                pause(5)
                Warning_flag2=Warning_flag2+1;
                try
                delete(hhh)
                catch
                end
%ZZ                ed=errordlg([{'OPAs are outside the domain'},{'Please review river input file or decrease the simulation time.'}],'Error');
%ZZ                set(ed, 'WindowStyle', 'modal');
%ZZ                uiwait(ed);
%ZZ                minDt = 0; %terminate the simulation
%ZZ                Exit=1;
%ZZ                return
            end
...
%ZZ            if time(t)>T2_Hatching*3600  %after hatching
%ZZ                Vzpart(a) = zeros(length(Vzpart(a)),1);
%ZZ                Vswim(a) = zeros(length(Vzpart(a)),1);
%ZZ            else %% if egg stage
%ZZ                Vswim(a) = zeros(length(Vzpart(a)),1);
%ZZ            end
            ...
            %ZZ Deposition and Resuspension
            if nnz(~a)>0 %ZZ if status of any particle is "deposit", check bed shear stress for resuspension
				k=logical((~a).*(ustar >= Ustarc)');    %ZZ deposit but bed shear stress >= critical shear stress
                kk=logical((~a).*(ustar < Ustarc)');    %ZZ deposit but bed shear stress < critical shear stress
             ...
             %   end
                ...
                Z(t,k) = -H(k)+d/2+Vs/1000*Dt; %ZZ assuming velocity upward is the same as settling velcocity, need to check later   
                Z(t,kk) = -H(kk)+d/2;    %ZZ If they were already dead,leave them in the bottom.
                alive(t,kk) = 0;	%ZZ status is "deposit"
			end
            ...
            while sum(btop)>0	%ZZ If it overpasses the top
                Z(t,btop) = -d-Z(t,btop);	%%ZZ reflect it
                b=Z(t,:)' < -H(:)+d/2;% Is any egg overpasses the bottom
                if sum(b) > 0 %if any egg touch the bottom get reflected...
                    %ZZ Z(t,b) = -Z(t,b)'-2*(H(b)-d/2);	%%ZZ reflect
					Z(t,b) = -H(b)+d/2;	%ZZ stay on bottom
					alive(t,b) = 0;	%ZZ deposit
            ...
            while sum(b)>0
                ...
                b=Z(t,:)'<-H(:)+d/2;%ZZ b becomes 0 if deposit
            end
            ...
            %% Alive or dead ??
    %ZZ        if alivemodel==0
    %ZZ            [alive] = mortality_model(alive,d,a);
    %ZZ        end %mortality model
            
        end
        %% DELETE the waitbar;
        delete(h)
        %%
        M = msgbox('Please wait FluOil is saving the results','FluOil','help');
        %%
        ...
        hFluOilGui = getappdata(0,'hFluOilGui');
        setappdata(hFluOilGui, 'outputfile', outputfile);
...
%         Depth_HEC_RAS=arrayfun(@(x) x.Riverinputfile(1,3), HECRAS_data.Profiles);
        ...
        % hFluOilGui=getappdata(0,'hFluOilGui');
        % setappdata(hFluOilGui, 'Folderpath', Folderpath);
        ...
        function [Kprime,Kz]=calculateKz
            ...
            % In FluOil Z=0 is the water surface and
...
% 4.  Check_if_egg_isin_newcell                                                                %
    function Check_if_egg_isin_newcell_or_New_Hydraulic_time_step
        ...
        try % for unsteady input
            ...
            if time(t)+ HECRAS_FluOil_Timediff>=HECRAS_time_sec(HECRAS_time_counter+1)
...
%ZZ			  ed=errordlg([{'OPAs are outside the domain'},{'Please review river input file or decrese the simulation time.'}],'Error');
%ZZ                set(ed, 'WindowStyle', 'modal');
%ZZ                uiwait(ed);
%ZZ                minDt = 0; %terminate the simulation
%ZZ                Exit=1;
...
%ZZ                set(ed, 'WindowStyle', 'modal');
%ZZ                uiwait(ed);
%ZZ                msgbox(['Simulation time=', sprintf('%5.1f',time(t)/3600),'h, ','Mean X=',sprintf('%5.1f',mean(X(t-1,:))/1000),'km'],'FluOil message','help');
%ZZ				pause(2)
%ZZ                Exit=1;
%ZZ                return
%YL show a warning window and close it automatically after 3s
            ...
            Cell=cell(egg_index);%Cell is the cell where the current egg is located           
            [Vz,Vy,H,W,DH,ustar,T,KS,Vzpart,Egg_Direction]=update_local_Hydraulics_and_Temp_of_eggs(egg_index,Cell,Vz,Vy,H,W,DH,ustar,T,KS,Vzpart,Q,Egg_Direction);
            end
        end        
        ...
        function [Vz,Vy,H,W,DH,ustar,T,KS,Vzpart,Egg_Direction]=update_local_Hydraulics_and_Temp_of_eggs(egg_index,Cell,Vz,Vy,H,W,DH,ustar,T,KS,Vzpart,Q,Egg_Direction)
           %Vx(c(i))=VX(Cell); %m/s
            ...
            %ZZ fall velcocity is input and constant
            Vzpart(egg_index)=-Vs/1000; %ZZ mm/s to m/s
...
% 6.  Create hydraulic and QW variables
    function [CumlDistance,Depth,Q,Vmag,Vlat,Vvert,Ustar,Temp,Width,VX,ks]=Create_Update_Hydraulic_and_QW_Variables(HECRAS_time_index)
        ...
        if HECRAS_time_index<1||HECRAS_time_index>length(HECRAS_data.Profiles)
                ed=errordlg([{'HEC-RAS simulation time error'},{'Please review HEC-RAS simulation time and make sure it is long enough to perform FluOil simulation.'}],'Error');
                set(ed, 'WindowStyle', 'modal');
                uiwait(ed);
